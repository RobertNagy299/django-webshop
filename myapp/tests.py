...
class NewsletterAdminViewTest(TestCase):
    ...
    def test_post_send_newsletter_invalid_form(self):
        ...
        self.client.login(username='admin', password='password')
        response = self.client.post('/send-newsletter/', data={})  # empty form data
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Please fill out the required fields")
        
    def test_post_newsletter_without_cta_or_discount(self):
        """This function tests if the email is actually sent. Specifically, this function tests an email that doesn't have a call-to-action button"""
        data = {
            "subject": "Hello!",
...
class CartViewTests(TestCase):
    ...
    def test_cart_view_works(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'cart.html')  # adjust name if different
...
class ContactViewTests(TestCase):
    ...
    def test_contact_view_post_valid_data(self):
        ...
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(mail.outbox[0].subject, 'New business inquiry from test@example.com')
        self.assertTrue(Inquiry.objects.filter(email_of_sender='test@example.com').exists())   
...
class NewsletterViewTests(TestCase):
    ...
    def test_newsletter_post_valid_new_email(self):
        ...
        self.assertTrue(EmailList.objects.filter(email_address='newuser@example.com').exists())
    ...
    def test_newsletter_post_duplicate_email(self):
        ...
        EmailList.objects.create(email_address='already@there.com', first_name='Bob')
        data = {
    ...
    def test_newsletter_post_invalid_data(self):
        ...
        response = self.client.post(reverse('newsletter'), data)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Please provide both your email and first name.")
        self.assertEqual(len(mail.outbox), 0)
